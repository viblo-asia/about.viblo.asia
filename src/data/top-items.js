export default [
  {
    title: 'Người dùng có <strong>reputation</strong> cao nhất',
    titleEn: 'User with the highest <strong>reputation</strong>',
    belongs: 'Tran Duc Thang <small>@thangtd90<small/>',
    url: 'https://viblo.asia/u/thangtd90'
  },
  {
    title: 'Người dùng <strong>cao điểm nhất<strong> Viblo Code',
    titleEn: '<strong>highest user<strong> Viblo Code',
    belongs: 'king <small>@knight<small/>',
    url: 'https://code.viblo.asia/users/knight'
  },
  {
    title: 'Hacker <strong>"bá đạo"</strong> nhất Viblo CTF',
    titleEn: 'Hacker <strong>"best"</strong> Viblo CTF',
    belongs: '生きたい <small>@ngoctnq<small/>',
    url: 'https://ctf.viblo.asia/users/ngoctnq'
  },
  {
    title: 'Organization có nhiều <strong>bài viết</strong> nhất',
    titleEn: 'Organization with the most <strong>articles</strong>',
    belongs: 'Sun* Blockchain Team',
    url: 'https://viblo.asia/o/sun-blockchain-team'
  },
  {
    title: 'Người dùng có nhiều <strong>bài viết</strong> nhất',
    titleEn: 'Users with the most <strong>posts</strong>',
    belongs: 'Cuong Le Ngoc <small>@lncnb</small>',
    url: 'https://viblo.asia/u/lncnb'
  },
  {
    title: 'Bài viết được <strong>vote up</strong> nhiều nhất',
    titleEn: 'Articles with the most <strong>votes up</strong>',
    belongs: 'Docker: Chưa biết gì đến biết dùng (Phần 1- Lịch sử)',
    url: 'https://viblo.asia/p/docker-chua-biet-gi-den-biet-dung-phan-1-lich-su-ByEZkWrEZQ0'
  },
  {
    title: 'Bài viết <strong>được xem</strong> nhiều nhất',
    titleEn: '<strong>most viewed</strong> articles',
    belongs: 'Phân tích thiết kế hệ thống thông tin sử dụng biểu đồ UML (Phần 1)',
    url: 'https://viblo.asia/p/phan-tich-thiet-ke-he-thong-thong-tin-su-dung-bieu-do-uml-phan-1-PjxMe6yNG4YL'
  },
  {
    title: 'Bài viết <strong>được clip</strong> nhiều nhất',
    titleEn: 'Article <strong>most clipped</strong>',
    belongs: 'Học Regular Expression và cuộc đời bạn sẽ bớt khổ (Updated v2.2)',
    url: 'https://viblo.asia/p/hoc-regular-expression-va-cuoc-doi-ban-se-bot-kho-updated-v22-Az45bnoO5xY'
  },
  {
    title: '<strong>Tag</strong> được nhiều người dùng <strong>follow</strong> nhất',
    titleEn: '<strong>Tag</strong> most users <strong>follow</strong>',
    belongs: 'JavaScript',
    url: 'https://viblo.asia/tags/javascript'
  },
  {
    title: '<strong>Tag</strong> có nhiều bài viết nhất',
    titleEn: '<strong>Tag</strong> has the most posts',
    belongs: 'Ruby on Rails',
    url: 'https://viblo.asia/tags/ruby-on-rails'
  }
]
