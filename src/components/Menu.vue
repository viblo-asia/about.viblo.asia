<template>
  <div id="links">
    <nav id="menu" class="flex items-center justify-between flex-wrap bg-teal-500 p-6 fixed z-50 w-full">
      <div class="flex items-center flex-shrink-0 text-white mr-6">
        <!-- logo -->
        <svg class="fill-current h-8 w-24 mr-2" viewBox="0 0 321.74 107.65" xmlns="http://www.w3.org/2000/svg">
         <!-- letter V -->
      ...
      <div class="block xl:hidden">
        <button class="btn-menu flex items-center px-3 py-2 border rounded text-teal-200 border-teal-400 hover:text-white hover:border-white" @click="showMenu = !showMenu">
          <svg class="fill-current h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Menu</title><path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"/></svg>
        </button>
      </div>
      <div id="menu-items" class="w-full xl:block flex-grow xl:flex xl:items-center xl:w-auto xl:text-right" :class="showMenu ? 'block' : 'hidden'">
        <div class="text-sm xl:flex-grow font-bold text-white">
          <a v-for="(item, index) in dataMenu" :key="index" href="javascript:void(0)" @click="scrollTo(item.code); showMenu = false" class="block mt-4 xl:inline-block xl:mt-0 text-teal-200 hover:text-white mr-4 uppercase">
            {{ item.title }}
          </a>
        </div>
      </div>
